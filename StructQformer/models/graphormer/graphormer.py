...
class BertSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        ...
        # added
        self.dist_bias = nn.Embedding(64, self.num_attention_heads)
        self.dist_bias.weight.data.fill_(0)
...
class Graphormer(nn.Module):
    ...
    def forward(self, graphs):
        graph_attention_mask = graphs["graph_attention_mask"]

        if "node_token_ids" in graphs:
            node_ids = graphs["node_token_ids"]
        ...
        # node_embeds += self.node_type_embedding(graphs['node_types'])
        # node_embeds = self.dropout(self.LayerNorm(node_embeds))
